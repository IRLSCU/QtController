...
void NmeaPres::gga_callback(std::string NmeaType, void * ggaStruct, void * context)
{
	...
	if(gga != 0) {
		((GlobalGpsStruct *)(context))->fAltitude = gga->altitude;//change，原有代码
		((GlobalGpsStruct *)(context))->quality = gga->quality;
		((GlobalGpsStruct *)(context))->fLatitude = gga->latitude;
		((GlobalGpsStruct *)(context))->fLongitude = gga->longitude;
		((GlobalGpsStruct *)(context))->ulTime = gga->time;
	}
}
...
//{
//	NmeaPres * NewParser = new NmeaPres("/dev/ttyO0", 4800);
//    if(NewParser->NmeaInitParsers() == false)
//		delete NewParser;
//
//	std::string GpsFrame;
//	std::ifstream infile("./nmea0183.txt");
//	if(!infile.is_open()) {
...
//	std::cout << "Quality: "	 << GlobalGps.quality		<< std::endl;
//	delete NewParser;
//	return 0;
//}
