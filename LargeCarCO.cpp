...
LargeCarCO::~LargeCarCO() {}
LargeCarCO &LargeCarCO::setTurnRange(qint16 turnRange)
{ //0~15位,第0、1个字节
...
LargeCarCO &LargeCarCO::setSpeed(quint8 speed)
{   //16~23位,第2个字节.0表示最大加速度，127表示最小加速度
...
LargeCarCO &LargeCarCO::setBrake(quint8 brake)
{   //16~23位，第2个字节，128最小减速度，255最大减速度
...
*/
LargeCarCO &LargeCarCO::setGear(quint8 gear)
{ //24~31，第3个字节
...
*/
LargeCarCO &LargeCarCO::setSignal(quint8 signal)
{ //32-39,第4个字节
...
（1）代表鸣笛；(0)代表静音
*/
LargeCarCO &LargeCarCO::setHorn(quint8 horn)
{ //40-47,第5个字节
...
void LargeCarCO::printInfo()
{
    qDebug("Large Car Control Order: speed:%d,turnRange:%d,gear:%d%s,lightSignal:%d%s,hore:%d%s",
           speed, turnRange,
           gear, gear == 0 ? QStringLiteral("空挡") : gear == 1 ? QStringLiteral("前进挡")
                                                                : QStringLiteral("后退档"),
           signal, signal == 0 ? QStringLiteral("所有灯光关闭") : signal == 1 ? QStringLiteral("右转向灯亮")
                                                                              : QStringLiteral("左转向灯亮"),
