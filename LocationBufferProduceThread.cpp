...
#include <QDebug>
#include <string.h>
LocationBufferProduceThread::LocationBufferProduceThread(CharRingBuffer* ringBuffer,LocationRingBuffer* loactionRingBuffer, QObject *parent = 0, QString name=tr("")):QThread(parent){
...
void LocationBufferProduceThread::run(){//将char缓冲区中的数据拼接成gpsInfo类型数据，并存入环形缓冲区
    m_isCanRun = true;
