...
bool GPSProcesser::initRoute(std::string path) {
	...
	double lon=0;
	double lat = 0;
	while (!fin.eof()) {//将经纬度转换成高斯坐标下的点
		fin >> lon >> lat;
		LongLat pos;
		pos.Lon = lon; 
		pos.Lat = lat;
        QPointF point = cCoordinate.LongLat2XY(pos);
        gps_route.push_back(GaussGPSData(point.x(), point.y(), FIXED_SOLUTION));
	}
...
double GPSProcesser::getPointDistance(GaussGPSData start, GaussGPSData end) {
	double distance = sqrt(pow((start.m_x - end.m_x), 2.0) + pow(start.m_y - end.m_y, 2.0));
	return distance;
}

double GPSProcesser::getLineDistance(Line line) {
	return getPointDistance(line.m_start, line.m_end);
}

bool GPSProcesser::isInArea(const GaussGPSData source, const GaussGPSData destination, const double range) {//判断是否到达下个点
	return getPointDistance(source, destination) <= range;
}

double GPSProcesser::calPointFromLineDistance(const GaussGPSData point, const Line line) {
	//Ax+By+C=0;
...
/*
	...
	转化成向量AB与向量AC的向量积
*/
double GPSProcesser::calPointFromLineDirector(const GaussGPSData point, const Line line) {
...
double GPSProcesser::pointProduct(Line line1, Line line2) {
...
GaussGPSData GPSProcesser::gpsData2Gauss(double gpslongitude, double gpslatitude, double quality) {
    QPointF xyPoint = cCoordinate.LongLat2XY(gpslongitude, gpslatitude);
    return GaussGPSData(xyPoint.x(), xyPoint.y(), quality);
}
bool GPSProcesser::pointsInSameSide(GaussGPSData point1, GaussGPSData point2, Line line) {
	return calPointFromLineDirector(point1, line) == calPointFromLineDirector(point2, line);
}
/*
	垂直于line，并且过point点
*/
Line GPSProcesser::getCorssLine(Line line, GaussGPSData point) {
...
/*
	line1 到line2的转向角
*/
double GPSProcesser::calLinesAngle(Line line1, Line line2) {
...
k1到k2的转向角
*/
double GPSProcesser::calLinesAngle(double k1, double k2) {
...
double GPSProcesser::calLineK(Line line) {
