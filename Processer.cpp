...
double Processer::startProcess2(GaussGPSData current)
{
    ...
    {
        if (isInArea(current, gps_route[i], IN_DISTANCE))
        { //判断是否到了下一个GPSRoute上的点，方法一：通过是否靠近下个点IN_DISTANCE距离。
    ...
    if (flag == 0)
    { //车辆未靠近任意一点.方法二：当前GPS位置和上一次获取的GPS点，是否均在目标点和目标点的下一个点的连接线一侧上，若在两侧，则说明应该将当前点设置为目标点，目标点设计为当前目标点的下一个。
        if (current_point_count + 2 < sum_gps_point)
        {                                                                                                                        //若已在当前路径下的最后第二个点，则无法通过方法二判断。
...
bool Processer::isInArea(const GaussGPSData source, const GaussGPSData destination, const double range)
{ //判断是否到达下个点
...
{
    double pointProduct = (line1.m_end.m_x - line1.m_start.m_x) * (line2.m_end.m_x - line2.m_start.m_x) + (line1.m_end.m_y - line1.m_start.m_y) * (line2.m_end.m_y - line2.m_start.m_y);
    return pointProduct;
}
